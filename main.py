...
import clip
from newt_main.benchmark.pt_resnet_feature_extractor import load_feature_extractor
from newt_main.benchmark.configs import model_specs
...
stage = 'iNat2021_sup' # no_pretrain, iNat2021_sup
    ...
    for k in kk:
        ...
        for experiment in range(number_of_exp):
            ...
            if stage in ['no_pretrain', 'check_clip']:
                ...
                model = Model(model_name, num_classes, stage, device, path, k)
            elif stage == 'iNat2018_BBN':
                path = f"{root}/BBN.iNaturalist2018.res50.180epoch.best_model.pth"
                model = Model(model_name, num_classes, stage, device, path, k)
            elif stage == 'iNat2021_sup':
                model_spec = model_specs[8]
                model = load_feature_extractor(model_spec, device)
            else:
